...
class SandboxSkill(Skill):
    ...
    def stop(self):
        subprocess.run([self.runtime, "container", "rm", "-f", self.container_name], capture_output=True)
    ...
    async def _ensure_container(self):
        ...
        inspect = await self._run(
            [self.runtime, "container", "inspect", "--format",
             '{{.State.Running}}\n{{range .Mounts}}{{.Source}}\t{{.Destination}}\n{{end}}',
             self.container_name],
            capture_output=True, text=True, encoding="utf-8",
        )
        if inspect.returncode != 0:
            img = await self._run([self.runtime, "image", "inspect", env_image], capture_output=True)
            ...
            if run.returncode != 0:
                raise RuntimeError(f"docker run failed ({run.returncode}): {run.stderr.strip()}")
            ...
            if not running:
                await self._run([self.runtime, "container", "start", self.container_name], check=True)
                ...
                await self._run([self.runtime, "container", "rm", "-f", self.container_name], capture_output=True)
