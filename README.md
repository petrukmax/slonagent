...
| `SmartCompressor` | Порт ReMe (Alibaba) — компактизация, сжатие и выгрузка сообщений через LLM |
...
Накапливает статистику по каждому инструменту (число вызовов, success rate, среднее время, средний расход токенов) и генерирует обогащённое описание через LLM на основе реальных примеров использования из диалога.
