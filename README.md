...
Telegram-агент на базе Gemini с многослойной долгосрочной памятью, планировщиком и скиллами.
...
`LogCompressor` — реализация [Mastra Observational Memory](https://mastra.ai/docs/memory/overview) с теми же промптами, адаптированная для работы с Gemini через google-genai SDK.
...
Hindsight — хорошо спроектированная система факторной памяти от vectorize.io, но требует запущенного внешнего сервера. Здесь вся логика встроена напрямую, промпты и алгоритмы 1:1, никаких внешних процессов — всё работает из коробки.
...
Два режима в зависимости от того, нужен ли доступ к хосту.

**Sandbox (изолированный контейнер)**

Основной режим. Код выполняется в Podman-контейнере (`python:3.11-slim`), изолированно от хоста. Контейнер персистентный — установленные пакеты и файлы в `/workspace` сохраняются между вызовами.
...
Скрипты в `/workspace/tools/` (`.py`, `.sh`) автоматически становятся инструментами с префиксом `script_` — агент создаёт переиспользуемые утилиты прямо в диалоге.

**SkillWriter (доступ к хосту с одобрения)**

Если нужно то, что из контейнера недоступно — температура дисков, статус системных процессов, доступ к железу — агент предлагает написать Python-скилл, который запустится прямо в процессе агента на хосте.

Сценарий: агент пишет код, отправляет его пользователю на проверку, пользователь смотрит и пишет `/approve_skill disk_temp` — скилл активируется и становится постоянным инструментом. Без одобрения код не выполняется.

Так агент получает доступ к хосту, но не самовольно — только через явный approval от пользователя.
...
### Горячая загрузка скиллов

`SkillWriterSkill` позволяет написать новый скилл прямо в диалоге и загрузить его без перезапуска. Скилл сохраняется в `memory/skills/` и переживает рестарт.
...
`env` — переменные окружения, задаются до старта (прокси, HuggingFace offline-режим и т.д.). Синтаксис `$keys.gemini` в других секциях — ссылка на значение из этого же конфига. Полный пример со всеми секциями — в `.config.sample.json`.
...
memory/
  ...
  skills/             ← динамически загруженные скиллы
